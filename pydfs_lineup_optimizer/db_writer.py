...
query = "INSERT INTO DK_Lineups (LineupID, GameID, Lineup, Projection, Salary)\
     VALUES(%s, %s, %s, %s, %s);"

def PrintLineups(LP, fileName):
    ...
    for each in LP:                    
        ...
        vals = [lineupID, GameID, json.dumps(lineups), each.fantasy_points_projection, each.salary_costs]
...
def get_json(player):
    ...
    player_json["NAME + ID"] = player_json["Name"] + "(" + player.id + ")"
